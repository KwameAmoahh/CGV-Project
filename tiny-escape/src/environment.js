...
export class Environment {
  ...
  loader.load('assets/models/full_kitchen.glb', (gltf) => {
    ...
    // Prepare door-related skip filters
    const DOOR_KEYWORDS = ['door', 'handle', 'hinge', 'holder'];
    const HUMAN_HIDE_TAGS = [
      'human','character','person','people','man','woman',
      'male','female','chef','worker','tpose','mixamo','armature'
    ];
    const STRUCTURE_KEYWORDS = ['wall','shelf','tray','rack','drawer','counter','cabinet','floor','base','fridge','interior','side','panel','support','divider','frame'];
    ...
    root.traverse((o) => {
      ...
      if (nm.includes('fridge') || nm.includes('refrigerator') || nm.includes('fridge_body') || nm.includes('fridge_interior')) {
        ...
        if (!box.isEmpty()) {
          const vol = box.getSize(new THREE.Vector3()).x * box.getSize(new THREE.Vector3()).y * box.getSize(new THREE.Vector3()).z;
          fridgeCandidates.push({ obj: o, vol });
        }
      }
    });
    ...
    // Traverse all meshes in the GLTF
    root.traverse((obj) => {
      if (obj.isMesh) {
        obj.castShadow = true;
        ...
        if (isNamedLike(name, DOOR_KEYWORDS) && this._isDescendantOf(obj, scanRoot)) {
          if (!this.doorMesh) {
            this.doorMesh = obj;
            this._doorClosedRotationY = this.doorMesh.rotation.y;
          }
          this._doorColliders.push(obj);
          this.collisionManager.addCollidableObject(obj);
          const doorBox = new THREE.Box3().setFromObject(obj);
          if (!doorBox.isEmpty()) this._blockingBoxes.push({ box: doorBox, ref: obj, tag: 'door' });
          return;
        }
        ...
        // Interactives (skip door entirely)
        if (!this.buttonBox && isNamedLike(name, ['button', 'switch'])) {
          const box = new THREE.Box3().setFromObject(obj);
          this.buttonBox = box;
        }

        // Blockers (skip doors, handles, hinges, holders)
        if (!isNamedLike(name, [
          'door', 'handle', 'hinge', 'holder',
          'light', 'bulb', 'vent_fan', 'can', 'soda', 'bottle', 'egg', 'carton', 'plate', 'dish', 'food',
          'human','character','person','people','man','woman','male','female',
          'chef','worker','tpose','mixamo','armature'
        ])) {
          const box = new THREE.Box3().setFromObject(obj);
          if (box.isEmpty()) return;
          this._blockingBoxes.push({ box, ref: obj });
          if (this._isDescendantOf(obj, scanRoot)) {
            const size = box.getSize(new THREE.Vector3());
            const largest = Math.max(size.x, size.y, size.z);
            if (largest > 0.15 || isNamedLike(name, STRUCTURE_KEYWORDS)) {
              this.collisionManager.addCollidableObject(obj);
            }
          }
        }
        ...
        // Record for later geometry analysis
        if (this._isDescendantOf(obj, scanRoot)) {
  ...
  resolveCollision(current, desired, radius = 0.25, height = 1.2, quaternion = new THREE.Quaternion()) {
    if (!this.loaded) return desired.clone()

    if (this.collisionManager) {
      if (!this.collisionManager.checkCollision(desired, quaternion)) {
        return desired.clone()
      }

      const tryX = new THREE.Vector3(desired.x, desired.y, current.z)
      if (!this.collisionManager.checkCollision(tryX, quaternion)) {
        return tryX
      }

      const tryZ = new THREE.Vector3(current.x, desired.y, desired.z)
      if (!this.collisionManager.checkCollision(tryZ, quaternion)) {
        return tryZ
      }
    }
    ...
    const collides = (p) => {
      for (const blocker of this._blockingBoxes) {
        if (pointToBox2D(p, blocker.box)) return true
      }
      return false
    }

    if (!collides(result)) return result
  ...
  openDoor() {
    if (this.doorMesh) {
      ...
      if (!this._doorBlockerRemoved) {
        const doorSet = new Set(this._doorColliders)
        this._blockingBoxes = this._blockingBoxes.filter(b => !doorSet.has(b.ref))
        for (const mesh of this._doorColliders) {
          this.collisionManager.removeCollidableObject(mesh)
        }
